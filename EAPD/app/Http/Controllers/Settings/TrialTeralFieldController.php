...
class TrialTeralFieldController extends Controller
{
    protected $active;
    ...
    {
        $this->active = awtTrans('مجالات التعاون');
        $this->view = 'Fixed';
        $this->route = 'trial_terals_fields';
        $this->form = FixedForm::class;
        $this->model = TrialTeralField::class;
        $this->title = 'trial_terals_fields';
        $this->keys = [
            'name_ar',
            'name_en',
    ...
    public function datatable()
    {
        ...
        return DataTables::of($model)->addColumn('action', function ($model) {
            ...
            $data .= '<a data-delete-url="'.route($this->route.'.delete', [$model->id]).'" style="    margin-right: 10px;" class="btn btn-sm btn-danger delete-row"><i class="vl_recycle-bin"></i>'.trans('main.delete').'</a>';
