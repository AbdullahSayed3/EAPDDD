...
use App\Forms\Settings\NameForm;
...
class AidsSuppController extends Controller
{
    protected $active;
    ...
    {
        $this->active = awtTrans('موردين المنح');
        $this->view = 'Fixed';
        $this->route = 'aids_suppliers';
        $this->form = NameForm::class;
        $this->model = AidSupplier::class;
        $this->title = 'aids_suppliers';
        $this->keys = [
            'name',

        ];
        $this->table = [
            ['name' => 'name', 'display' => trans('main.name'), 'data' => 'name', 'orderable' => 'true', 'searchable' => 'true'],
    ...
    public function index(): View
    {
        ...
        return view($this->view.'.aid_supplier', compact('active', 'table_rows', 'route'));
    }
    ...
    public function show(int $id, FormBuilder $formBuilder)
    {
        ...
        $form = $formBuilder->create($this->form, [
            ...
            'url' => route($this->route.'.update', parameters: [ $model['id']]),
        ], ['id' => $model->id]);
