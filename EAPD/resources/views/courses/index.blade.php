...
@section('content')
    ...
    <!-- Page Title -->
    <x-base.breadcrumb title="{{ 'الدورات التدريبية' }}" :breadcrumbs="[['label' => 'الصفحة الرئيسية', 'url' => route('home')], ['label' => 'الدورات التدريبية']]" />

    <!-- start search -->
    @include('courses.filter')
    <!-- end search -->

    <!-- begin table -->
    <form method="post" action="{{ route('courses.delete', $dataT) }}">
        @csrf
        <div class="row">
            <div class="col-xl-12">
                ...
                <div class="mb-4">
                    <div class="border-0">
                        <div class="row">
                            <div class="col-md-12">
                                @can('create_courses')
                                    <x-base.fillBtn :href="route($route . '.create')" icon="fa-plus-square-o" label="{{ awtTrans('اضافة') }}"
                                        float="start" />
                                @endcan
                                ...
                                <x-base.fillBtn id="deleteBtn" style="display:none" modalTarget="deleteModal"
                                    modalToggle="true" class="btn-danger" float="end" label="{{trans('awt.delete')}}" badge="true"
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" class="form-control" id="print_title"
                                                        name="print_title" placeholder="{{ awtTrans('عنوان التقرير') }}">
                                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#reset-search').on('click', function(e) {
                ...
                $('.selc_country, .selc_cour_typ, .selc_cour_train').val(null).trigger('change');
            });

            // Handle Clear Advanced Search Form
            $('#clear-advanced-search').on('click', function() {
                $('#advanced-search-form')[0].reset();
                $('.selc_country, .selc_cour_typ, .selec_cours_typ, .selec_cours_typ3, .selc_cour_train, .selc_cours_cord')
            ...
            $(".selc_cour_typ, .selec_cours_typ").select2({
