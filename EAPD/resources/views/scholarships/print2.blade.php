            ...
            <div class="data-table">
                <table class="table table-bordered  " cellspacing="0" style="text-align: center;" >
                    ...
                    <tr>
                        <td>{{ $t }}</td>
                        @php($t++)
                        @if(in_array('program', $print_choices))
                        <td>{{$model->program}}</td>
                        @endif
                        @if(in_array('owner', $print_choices))
                        <td>{{ $model->owner }}</td>
                        @endif
                        @if(in_array('start_date', $print_choices))
                        <td>{{$model->start_date->format('Y-m-d') }}</td>
                        @endif
                        @if(in_array('end_date', $print_choices))
                        <td>{{ $model->end_date->format('Y-m-d')  }}</td>
                        @if(in_array('learners', $print_choices))
                        <td>{{ $model->learners->count() }}</td>
                        @endif
                        @endif


                        @if(in_array('participants', $print_choices))
                        <td>@foreach (unserialize($model->participants) as $row)
                            {{ getCountry($row) }}<br>
                        @endforeach </td>
                        @endif
                        @if(in_array('department', $print_choices))
