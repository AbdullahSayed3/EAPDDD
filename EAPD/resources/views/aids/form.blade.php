...
@section('content')
    <!--page title-->
    <x-base.breadcrumb :title="request()->routeIs('aids.create') ? 'إضافة منحة / معونة' : 'تعديل منحة / معونة'" :breadcrumbs="[
        ['label' => 'الصفحة الرئيسية', 'url' => route('home')],
        ['label' => 'فعاليات', 'url' => route($route . '.index')],
        ['label' => request()->routeIs('courses.create') ? 'إضافة منحة / معونة' : 'تعديل منحة / معونة'],
    ]" />
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6">
                                <select class="form-select " required name="type_id">
                                    {{-- <option selected >{{trans('awt.اختر نوع المنحة')}}</option> --}}
                                    @foreach (\App\Models\AidType::whereNull('parent_id')->get() as $item)
