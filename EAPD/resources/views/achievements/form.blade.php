...
@section('content')
    <!--page title-->
    <x-base.breadcrumb title=" {{request()->routeIs('roles.create') ?  awtTrans('create') .' '  : awtTrans('edit').' ' .  'الانجازات' }}" :translate="false" :breadcrumbs="[
        ['label' => 'الصفحة الرئيسية', 'url' => route('home')],
        ['label' => 'الانجازات', 'url' => route($route . '.index')],
        ['label' => request()->routeIs('roles.create') ? awtTrans('create') : awtTrans('edit') . 'الانجازات'],
    ]" />
